...
int main(int argc, char *argv[])
{
	...
	if (allocated > globalN) N -= (allocated - globalN);
	...
	for (int i = 0; i < iterations; i++)
	{
		if (rank < size - 1)
		{
			MPI_Sendrecv(GetLastRowArray(board, originalN, globalN), globalN, MPI_INT, rank + 1, 0,
			             board, globalN, MPI_INT, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		if (rank > 0)
		{
			MPI_Sendrecv(GetShiftedArray(board, rank, globalN), globalN, MPI_INT, rank - 1, 0,
			             GetBottomGhostRowArray(board, originalN, globalN), globalN, MPI_INT, rank, 0, MPI_COMM_WORLD,
			             MPI_STATUS_IGNORE);
		}
